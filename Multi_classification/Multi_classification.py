...
"""
进行"纲"和"种"模型的训练和验证，并调用训练好的模型进行预测
"""
...
def visualize_dataset():
    """
    随机可视化一个输出数据图片与对应标签，验证是否符合我们的设定
    :return:
    """
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for idx, data in enumerate(data_loaders[phase]):
                ...
                label_classes = data['classes'].to(device)
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    loss = (criterion(x_species, label_species) + criterion(x_classes, label_classes)) / 2
                    if phase == 'train':
                        loss.backward()
                        optimizer.step()
            ...
            if phase == 'val' and (epoch_acc_classes + epoch_acc_species > best_acc):
                best_class_acc = epoch_acc_classes
                best_species_acc = epoch_acc_species
                best_acc = epoch_acc_classes + epoch_acc_species
                best_model_wts = copy.deepcopy(model.state_dict())
                print('Best val classes Acc: {:.2%}, species Acc: {:.2%}'.format(best_class_acc, best_species_acc))
...
def visualize_model(model):
    ...
    with torch.no_grad():
        for i, data in enumerate(data_loaders['val']):
            ...
            plt.title('predicted classes: {} ground-truth classes:{}\n predicted species: {} ground-truth species:{}'
                      .format(CLASSES[preds_classes], CLASSES[labels_classes], SPECIES[preds_species], SPECIES[labels_species]))
            plt.show()
